...
@pytest.mark.asyncio
async def test_hybrid_search_service_uses_primary_reranker_scores() -> None:
    ...
    service = HybridSearchService(
        ...
        primary_reranker=cast(RemoteReranker, reranker),
        secondary_reranker=None,
        source_language="en",
    ...
    results = await service.search(query=" SAML VPN ", user_email="aida@example.com", user_groups=["employees"])
    ...
    assert results[0].rerank_provider == "bedrock-cohere"
    assert reranker.calls[0].documents == ["Vacation policy", "VPN setup with SAML"]
...
@pytest.mark.asyncio
async def test_hybrid_search_service_uses_openai_fallback_after_primary_rerank_failure() -> None:
    class FakeEmbeddingProvider:
        def embed_query(self, query_text: str) -> list[float]:
            return [0.1, 0.2]
...
@pytest.mark.asyncio
async def test_hybrid_search_service_keeps_rrf_order_when_rerankers_fail() -> None:
    class FakeEmbeddingProvider:
        def embed_query(self, query_text: str) -> list[float]:
            return [0.1, 0.2]
    ...
    results = await service.search(query="policy", user_email="aida@example.com", user_groups=["employees"])
