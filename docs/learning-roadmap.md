...
Build this project one working slice at a time. Do not start with the LLM. First make data flow through the system in a
boring, testable way.
...
Verify: write provider-contract tests with a fake Bedrock client first, then run one manual real-provider call when
